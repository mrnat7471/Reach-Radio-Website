<template>
  <div class="page" style="padding-bottom: 10px;">
      <h5 class="border3 ml-1 mr-1 pt-3">Schedule Today</h5>
    <section class="container">
      <div>
        <b-container fluid class="time">
          <b-row>
            <div :key="slot.uuid" v-for="slot in slots" class="col-sm-5 mt-3 mr-3" style="min-width:290px;border-radius:5px;" v-if="currentTime <= slot.end && slot.user.firstName != 'Non Stop'">
              <!-- Slot Div -->
              <b-row class="schedule home-pbox">
                ...
                <b-col class="timetable_slot">
                    <h5><br>
                      <div style="min-width: 145px;" v-if="slot.user.uuid != 'c4b3638a-074c-40e2-8c09-6dcef23ab0ff'"><div v-if="slot.user.uuid != '3d1921b2-6043-4418-8b04-501ea83b1a52'">{{slot.user.firstName + ' '+slot.user.lastName}}<br></div></div></h5>
