    ...
    <section class="container pt-3 pb-3">
      <div>
        <div v-if="versions.supported_game_version">
          <div class="pt-3 pb-3 text-white pl-3 pr-3 mt-3 mb-3 standardheader" id="news-title"
              style="display: flex; width: 100%;">
            <h4 style="padding-right: 10px;">Latest TruckersMP Information</h4>
          </div>
          <div>
            <b-row>
        ...
        <div v-else>
          ...
          <p style="text-align:center">The latest data isn't currently available!</p>
        </div>

        <div v-if="theplayers.servers[0]">
          <div class="pt-3 pb-3 text-white pl-3 pr-3 mt-3 mb-3 standardheader" id="news-title"
                  style="display: flex; width: 100%;">
            <h4 style="padding-right: 10px;">TruckersMP Traffic Powered By Trucky</h4>
          </div>
          
          <div class="grid-container2">
          <div v-for="server in theplayers.servers" v-if="server.shortname == 'EU SIM 1'">
              <div class="pt-3 pb-3 mb-1 traffic-page grid-item2">
                <div class="pl-3">
        ...
        <div v-else>
          ...
          <p style="text-align:center">There currently isn't any TruckersMP Traffic data available right now!</p>
        </div>
        ...
        <div v-if="events[0]">
          <b-row>
            ...
            <div class="pt-3 pb-3 text-white pl-3 pr-3 mt-3 mb-3 standardheader" id="news-title"
                style="display: flex; width: 100%;">
                <h4 style="padding-right: 10px;">TruckersMP Events Today</h4>
              </div>
            </b-col>
          </b-row>
          ...
          <b-row style="margin-bottom: 10px; margin-left: 5px; margin-right: 5px;" v-for="event in events" v-bind:key="event.id">
            <b-col class="mb-3">
            <div><span style="font-size: 1.17em;">{{event.name}} </span>
            <span class="buttonchange" style="background-color: #003d91;padding: 5px;border-radius: 5%;color: white!important;white-space: nowrap;" v-if="event.server.id !== 0">{{event.game}} - {{event.server.name}}<br></span>
            <span class="buttonchange" style="background-color: #003d91;padding: 5px;border-radius: 5%;color: white!important;white-space: nowrap;" v-else>{{event.game}} - To be determined<br></span>
        ...
        <div v-else>
          <div class="pt-3 pb-3 text-white pl-3 pr-3 mt-3 mb-3 standardheader" id="news-title" style="display: flex; width: 100%;">
            <h4 style="padding-right: 10px;">TruckersMP Events Today</h4>
          </div>
          <p style="text-align:center">There currently isn't anymore TruckersMP events today. Check back later</p>
        </div>
      </div>
    </section>
  </div>
