<template>
  <div class="page" style="overflow: hidden">
    ...
    <div style="border-bottom: 1px solid gray;" class="footer-section">
      <b-row class="mr-4">
        <b-col style="max-width:75px;">
          <img :src="live.song.graphic.medium" height=75 width=75>
        </b-col>
        <b-col class="ml-4 mt-2">
          <audio preload="none" :src="stationData.fields.stationStreamUrl" ref="radioPlayer" style="display: none;"></audio>
        ...
        <b-col style="max-width:50px;" class="mt-3 mr-2">
  ...
  export default {
    ...
    methods:{
      ...
      getNpInfo() {
        ...
        axios.get(`https://api.reachradio.co.uk/retrieve.php?service=${process.env.SERVICE}&url=song-history/now-playing`)
          .then((res) => {
            this.live = res.data;
          })
      },
      loadSongs(){
        axios.get(`https://api.reachradio.co.uk/retrieve.php?service=${process.env.SERVICE}&url=song-history`)
          .then((res) => {
    ...
    mounted() {
      ...
      socket.on('current-song-updated', (event) => {
        ...
        if ('mediaSession' in navigator) {
          ...
          if ('setPositionState' in navigator.mediaSession) {
            navigator.mediaSession.setPositionState({
              duration: this.live.song.durationMs/1000,
              playbackRate: 1,
              position: 0,
            });
          }
