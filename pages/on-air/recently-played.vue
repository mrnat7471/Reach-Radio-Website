<template>
  <div class="page">
    <section class="container pt-3">
      <div>
        <h4 class="border3 ml-1 mr-1 pt-5" style="margin-bottom:15px;">Recently Played</h4>
        <b-row align-h="center" style="padding-bottom: 50px;" v-if="songs[0]">
          <div v-for="item in songs">
          <div class="mb-2 ml-2">
            <div>
              <div class="home-pbox" style="width: 210px;min-height: 340px;border-radius:2.5%;margin:5px;">
                <a :href="item.song.extraInfo.trackUrl" v-if="item.song.graphic.large != null" target="_blank"><v-lazy-image v-if="item.song.graphic.large != null" src-placeholder="~assets/images/main/r-small.webp" :src="item.song.graphic.large" style="width: 210px; height: 210px; object-fit: cover;border-top-left-radius:2.5%;border-top-right-radius:2.5%;"></v-lazy-image></a>
                <v-lazy-image v-else src="~assets/images/main/r-small.webp" style="width: 210px; height: 210px; object-fit: cover"></v-lazy-image>
                <h6 class="mt-3 ml-2 mr-2" style="text-align: center; font-weight: bold;">{{item.song.title}}</h6><p style="text-align: center;margin-bottom:5px;">{{item.song.artist}}</p>
                <div style="text-align: center;">
                  <a :href="item.song.extraInfo.trackUrl" v-if="item.song.graphic.large != null" target="_blank">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        item: {
          song:{
            ...
            graphic:{
              large: "~assets/images/main/r-small.webp",
            }
          }
        }
      }
    ...
    mounted: function() {
      socket.on('song-history-updated', (event) => {
        this.songs = event;
      });
    },
    filters: {
    ...
    loadSongs() {
       return axios
         .get(`#`)
         .then(( res ) => {
           this.songs = res.data._embedded.slice(1,21);
         })
      },
    },
  }
